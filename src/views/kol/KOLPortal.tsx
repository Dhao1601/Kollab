  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Xin chào, {currentKOL.name}!</h2>
          <p className="text-sm text-slate-500 dark:text-slate-400">{currentKOL.handle} • {currentKOL.platform} • {currentKOL.followersDisplay} followers</p>
        </div>
        <div className="flex gap-2">
          <button className="relative p-2 rounded-xl hover:bg-slate-100/60 dark:hover:bg-slate-700/60 transition-colors">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <KPICard label="Tổng thu nhập" value={`${(currentKOL.totalEarned / 1000000).toFixed(1)}M`} suffix="VND"
          icon={<DollarSign className="w-5 h-5" />} accent="bg-gray-50"
        ...
        <KPICard label="Đang chờ thanh toán" value={`${(currentKOL.pendingPayment / 1000000).toFixed(1)}M`} suffix="VND"
          icon={<Clock className="w-5 h-5" />} accent="bg-gray-50"
          onClick={() => setKpiPopup('pending')} />
        <KPICard label="Thứ hạng hiện tại" value={`#${currentKOL.currentRank}`}
          icon={<Award className="w-5 h-5" />} accent="bg-gray-50"
        ...
        <KPICard label="Đã hoàn thành" value={kolTasks.filter(t => ['published', 'paid', 'metrics_approved'].includes(t.status)).length.toString()}
          icon={<CheckCircle2 className="w-5 h-5" />} accent="bg-gray-50"
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-emerald-50/80 dark:bg-emerald-900/20 rounded-xl p-4 text-center border border-emerald-200 dark:border-emerald-800/40">
          ...
          <p className="text-xs text-slate-400 mt-1">{paidPayments.length} khoản</p>
        </div>
        <div className="bg-brand-50/80 dark:bg-brand-900/20 rounded-xl p-4 text-center border border-brand-200 dark:border-brand-800/40">
          <p className="text-xs text-brand-500 dark:text-brand-400 mb-1">Tháng này</p>
          <p className="text-2xl font-bold text-teal-600">{(currentMonth.amount / 1000000).toFixed(1)}M ₫</p>
          <p className="text-xs text-slate-400 mt-1">Tháng 5/2026</p>
        </div>
      </div>

      {/* Bar chart */}
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4">
        <p className="text-sm font-semibold text-slate-900 dark:text-white mb-3">Thu nhập theo tháng</p>
        <div className="flex items-end gap-2 h-32">
          {monthlyData.map((m) => {
            const pct = maxAmount > 0 ? (m.amount / maxAmount) * 100 : 0;
            return (
              <div key={m.month} className="flex-1 flex flex-col items-center gap-1">
                <span className="text-[10px] text-slate-400">{m.amount > 0 ? `${(m.amount/1000000).toFixed(1)}M` : '-'}</span>
                <div className="w-full rounded-t-lg bg-teal-500 dark:bg-teal-600 transition-all" style={{ height: `${Math.max(pct, 4)}%` }} />
                <span className="text-[10px] text-slate-400">{m.month.replace('Tháng ', '')}</span>
              </div>
            );
          })}
        </div>
      ...
      <div className="space-y-2 max-h-[250px] overflow-y-auto">
        <p className="text-sm font-semibold text-slate-500 dark:text-slate-300">Lịch sử thanh toán</p>
        {paidPayments.map(p => (
          <div key={p.id} className="flex items-center justify-between p-3 bg-slate-50/50 dark:bg-slate-700/50 rounded-xl">
            <div>
            ...
            <p className="text-sm font-bold text-gray-600">+{p.paidAmount.toLocaleString()}₫</p>
          </div>
        ))}
      </div>
    </div>
...
function KPIPopupPendingPayments() {
  ...
  return (
    <div className="space-y-4">
      <div className="bg-amber-50/80 dark:bg-amber-900/20 rounded-xl p-4 border border-amber-200 dark:border-amber-800/40 text-center">
        ...
        <p className="text-xs text-slate-400 mt-1">{pendingPayments.length} khoản đang xử lý</p>
      </div>
      ...
      <div className="space-y-2 max-h-[350px] overflow-y-auto">
        {pendingPayments.map(p => {
          ...
          return (
            <div key={p.id} className="rounded border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700 p-4 space-y-2">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-semibold text-slate-900 dark:text-white">{p.campaignName}</p>
                  <p className="text-xs text-slate-400">Thù lao theo hợp đồng</p>
                </div>
                <p className="text-lg font-bold text-gray-600">{p.amount.toLocaleString()}₫</p>
              </div>
              ...
              {task && (
                <div className="pt-2 border-t border-amber-200/50 dark:border-amber-800/30">
                  <p className="text-xs text-slate-500">KPI: {task.kpiTarget.views >= 1000 ? Math.round(task.kpiTarget.views/1000)+'K' : task.kpiTarget.views} views • {task.kpiTarget.engagementRate}% ER</p>
                  {task.metrics.views > 0 && (
                    <p className="text-xs text-gray-600 mt-0.5">✓ Đã đạt KPI — Views: {task.metrics.views >= 1000 ? `${Math.round(task.metrics.views/1000)}K` : task.metrics.views} • ER: {task.metrics.engagementRate}%</p>
                  )}
                </div>
              )}
            </div>
...
function KPIPopupRanking() {
  const rankings = [
    ...
    { rank: 2, name: 'Minh Tech Review', platform: 'YouTube', followers: '850K', er: '5.2%', score: 91, trend: '-1', color: 'bg-slate-400', badge: '' },
    { rank: 3, name: 'Foodie Hà Nội', platform: 'TikTok', followers: '2.1M', er: '4.1%', score: 89, trend: '+1', color: 'bg-amber-700', badge: '' },
    { rank: 4, name: 'Gym Coach Nam', platform: 'Instagram', followers: '320K', er: '7.3%', score: 85, trend: '0', color: 'bg-slate-300', badge: '' },
    { rank: 5, name: 'Mỹ Phẩm Thật', platform: 'Facebook', followers: '500K', er: '3.9%', score: 80, trend: '+3', color: 'bg-slate-300', badge: '' },
  ];
  ...
  return (
    <div className="space-y-4">
      {/* Top KOL */}
      <div className="bg-gray-50 dark:bg-gray-700 rounded p-5 border border-gray-200 dark:border-gray-700 text-center">
        <div className="flex items-center justify-center gap-3 mb-2">
          <span className="text-3xl">{topKol.badge || '👑'}</span>
          <div>
            <p className="text-lg font-bold text-gray-600 dark:text-gray-300">{topKol.name}</p>
            <p className="text-sm text-gray-600 dark:text-gray-400">{topKol.platform} • {topKol.followers} • ER {topKol.er}</p>
          </div>
        </div>
        <div className="inline-flex items-center gap-1 bg-gray-100 dark:bg-gray-700 rounded-full px-3 py-1">
          <span className="text-sm font-bold text-gray-600 dark:text-gray-300">Score: {topKol.score}</span>
          <span className="text-xs text-gray-600">↑ {topKol.trend}</span>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
        <div className="overflow-x-auto">
          ...
          <tbody>
            {rankings.map((r) => (
              <tr key={r.rank} className={`border-b border-slate-50 dark:border-slate-700/50 ${r.name === currentKOL.name ? 'bg-teal-50/30 dark:bg-teal-900/10' : ''}`}>
                ...
                <td className="px-3 py-2.5 text-xs text-slate-500">{r.platform}</td>
                <td className="px-3 py-2.5 text-xs text-slate-700 dark:text-slate-200">{r.followers}</td>
                <td className="px-3 py-2.5 text-xs font-medium text-teal-600">{r.er}</td>
                <td className="px-3 py-2.5 text-sm font-bold text-slate-900 dark:text-white">{r.score}</td>
                <td className="px-3 py-2.5 text-xs font-medium">
                  ...
                  {r.trend === '0' && <span className="text-slate-400">—</span>}
                </td>
              </tr>
            ))}
          </tbody>
      ...
      {yourStats && yourStats.rank !== 1 && (
        <div className="bg-gray-50 dark:bg-gray-700 rounded p-3 border border-gray-200 dark:border-gray-700 text-center">
          <p className="text-xs text-gray-600 dark:text-gray-400">Cách top 1</p>
          <p className="text-sm font-bold text-gray-600 dark:text-gray-400">{topKol.score - yourStats.score} điểm</p>
        </div>
      )}
    </div>
  );
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-4 gap-2">
        {[
          ...
          <div key={s.label} className="bg-slate-50/50 dark:bg-slate-700/50 rounded-xl p-3 text-center">
      ...
      <div className="space-y-3 max-h-[350px] overflow-y-auto">
        {completedTasks.map(task => (
          <div key={task.id} className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4 space-y-2">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-semibold text-slate-900 dark:text-white">{task.campaignName}</p>
                <p className="text-xs text-slate-400">{task.productName} • {task.brandName}</p>
              </div>
              <div className="text-right">
                <p className="text-sm font-bold text-gray-600">{task.payment.toLocaleString()}₫</p>
                <Badge label={task.status === 'paid' ? 'Đã thanh toán' : 'Hoàn thành'}
                  colorClass={task.status === 'paid' ? 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300' : 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300'} />
              </div>
            </div>
            {task.metrics.views > 0 && (
...
function KPICard({ label, value, suffix, icon, accent = 'bg-gray-50', onClick }: {
  ...
  return (
    <motion.div whileHover={{ y: -2 }} onClick={onClick}
      className={`bg-gradient-to-br ${accent} to-transparent rounded-2xl p-4 border border-slate-200/60 dark:border-slate-700/60 bg-white dark:bg-slate-800/80 ${onClick ? 'cursor-pointer hover:shadow-sm transition-shadow' : ''}`}>
      <div className="flex items-start justify-between">
        <div>
          <p className="text-xs text-slate-500 dark:text-slate-400 mb-0.5">{label}</p>
          <p className="text-xl font-bold text-slate-900 dark:text-white">{value}{suffix && <span className="text-xs font-normal text-slate-400 ml-0.5">{suffix}</span>}</p>
        </div>
        <div className="p-2 rounded bg-gray-50 dark:bg-gray-700 text-gray-600 dark:text-gray-400">{icon}</div>
      </div>
    </div>
  );
}
...
function DashboardView({ navigateTo, onSelectTask }: {
  ...
  return (
    <div className="space-y-6">
      {/* Quick Nav */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <QuickNavCard icon={<Search className="w-5 h-5" />} label="Khám phá chiến dịch" count={exploreCampaigns.length} color="text-blue-600" bg="bg-blue-50/80 dark:bg-blue-900/20"
          onClick={() => navigateTo('explore')} />
        <QuickNavCard icon={<FileText className="w-5 h-5" />} label="Nhiệm vụ của tôi" count={kolTasks.length} color="text-blue-600" bg="bg-blue-50/80 dark:bg-blue-900/20"
          onClick={() => navigateTo('tasks')} />
        <QuickNavCard icon={<Wallet className="w-5 h-5" />} label="Ví & Thanh toán" count={kolPayments.filter(p => p.status === 'pending').length} color="text-amber-600" bg="bg-amber-50/80 dark:bg-amber-900/20"
          onClick={() => navigateTo('wallet')} />
        <QuickNavCard icon={<Settings className="w-5 h-5" />} label="Cài đặt" count={0} color="text-slate-500" bg="bg-slate-50/50 dark:bg-slate-700/50"
      ...
      {(pendingTasks.length > 0 || revisionTasks.length > 0) && (
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => navigateTo('tasks')} className="text-xs text-gray-600 hover:underline font-medium">Xem tất cả →</button>
          </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={() => navigateTo('explore')} className="text-xs text-gray-600 hover:underline font-medium">Xem thêm →</button>
        </div>
      ...
      {kolTasks.filter(t => t.metrics.views > 0).length > 0 && (
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => navigateTo('tasks')} className="text-xs text-gray-600 hover:underline font-medium">Chi tiết →</button>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {kolTasks.filter(t => t.metrics.views > 0).slice(0, 3).map(task => (
              <div key={task.id} className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200/80 dark:border-slate-700/60 shadow-soft p-4">
                <p className="text-sm font-semibold text-slate-900 dark:text-white truncate mb-2">{task.campaignName}</p>
                <div className="grid grid-cols-3 gap-2 text-center">
                  <div>
                    <p className="text-base font-bold text-slate-900 dark:text-white">{task.metrics.views >= 1000 ? `${Math.round(task.metrics.views/1000)}K` : task.metrics.views}</p>
...
function QuickNavCard({ icon, label, count, color, bg, onClick }: {
  ...
  return (
    <motion.button whileHover={{ y: -2 }} onClick={onClick}
      className={`${bg} rounded-2xl p-4 border bg-slate-200 dark:border-slate-600 text-left hover:shadow-sm transition-all flex items-center gap-3`}>
      <div className={`${color}`}>{icon}</div>
      <div className="flex-1 min-w-0">
        <p className="text-sm font-semibold text-slate-900 dark:text-white truncate">{label}</p>
        {count > 0 && <p className="text-xs text-slate-500">{count} mục</p>}
      </div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <h3 className="text-xl font-bold text-slate-900 dark:text-white">Khám phá chiến dịch</h3>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {exploreCampaigns.map(c => (
          <div key={c.id} className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200/80 dark:border-slate-700/60 shadow-soft overflow-hidden">
            <img src={c.productImage} alt={c.productName} className="w-full h-36 object-cover" />
            <div className="p-4 space-y-2">
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-xs text-brand-500 dark:text-brand-400 font-medium">{c.brandName}</p>
                  <h4 className="font-semibold text-slate-900 dark:text-white">{c.productName}</h4>
                </div>
                <Badge label={c.category} colorClass="bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300" />
              </div>
              <p className="text-xs text-slate-500 line-clamp-2">{c.brief}</p>
...
function ExploreCard({ campaign }: { campaign: typeof exploreCampaigns[0] }) {
  return (
    <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200/80 dark:border-slate-700/60 shadow-soft overflow-hidden flex">
      ...
      <div className="p-3 flex-1 min-w-0">
        ...
        <p className="text-sm font-semibold text-slate-900 dark:text-white truncate">{campaign.productName}</p>
        <div className="flex items-center gap-2 mt-1 text-xs text-slate-400">
          ...
          <span>•</span>
          <span className="font-bold text-gray-600">{campaign.reward.toLocaleString()}₫</span>
        </div>
      </div>
    </div>
  );
...
function CampaignDetailModal({ campaignId, onClose }: {
  ...
  if (!campaign) return <p className="p-4 text-sm text-slate-500">Không tìm thấy chiến dịch.</p>;
  const [registered, setRegistered] = useState(false);

  return (
    <div className="space-y-4">
      <img src={campaign.productImage} alt={campaign.productName} className="w-full h-40 object-cover rounded" />
      <div className="flex items-start justify-between">
        <div>
          <p className="text-sm font-semibold text-brand-500 dark:text-brand-400">{campaign.brandName}</p>
          <h3 className="text-lg font-bold text-slate-900 dark:text-white">{campaign.productName}</h3>
        </div>
        <Badge label={campaign.status === 'open' ? 'Đang tuyển' : campaign.status} colorClass="bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300" />
      </div>
      ...
      <div className="space-y-3">
        <div className="bg-blue-50/80 dark:bg-blue-900/20 rounded-xl p-4">
          ...
          <p className="text-sm text-slate-600 dark:text-slate-300">{campaign.brief}</p>
        </div>
        <div className="bg-slate-50/50 dark:bg-slate-700/50 rounded-xl p-4">
          <h4 className="text-sm font-semibold text-slate-500 dark:text-slate-300 mb-1.5 flex items-center gap-1.5"><CheckCircle2 className="w-4 h-4" />Yêu cầu nội dung</h4>
          <p className="text-sm text-slate-600 dark:text-slate-300">{campaign.requirements}</p>
        </div>
        <div className="flex items-center justify-between p-3 bg-slate-50/50 dark:bg-slate-700/50 rounded-xl">
          <span className="text-xs text-slate-500">Giá trị sản phẩm mẫu</span>
          <span className="text-sm font-bold text-slate-900 dark:text-white">{campaign.productValue.toLocaleString()}₫</span>
        </div>
      </div>
      ...
      {!registered ? (
        ...
        <div className="space-y-3">
          <div className="p-3 bg-emerald-50/80 dark:bg-emerald-900/20 rounded-xl border border-emerald-200 dark:border-emerald-800/40 text-sm text-emerald-700 dark:text-emerald-300">
...
function TasksView({ onSelectTask }: { onSelectTask: (id: string) => void }) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
      <h3 className="text-xl font-bold text-slate-900 dark:text-white">Nhiệm vụ của tôi</h3>
        ...
        {[
          ...
          <button key={f.id} onClick={() => setFilter(f.id as any)}
            className={`px-3 py-1.5 rounded-xl text-sm font-medium transition-colors ${
              filter === f.id ? 'bg-teal-600 text-white' : 'bg-slate-100/60 dark:bg-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-600'
            }`}>
            {f.label} ({f.count})
          </button>
        ))}
...
function TaskCard({ task, onClick }: { task: typeof kolTasks[0]; onClick: () => void }) {
  ...
  return (
    <motion.div whileHover={{ y: -2 }} onClick={onClick}
      className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200/80 dark:border-slate-700/60 shadow-soft p-5 cursor-pointer hover:shadow-sm transition-all">
      <div className="flex items-start justify-between mb-3">
        <div>
          <p className="font-semibold text-slate-900 dark:text-white">{task.campaignName}</p>
          <p className="text-xs text-slate-500">{task.productName}</p>
        </div>
        <Badge label={taskStatusLabels[task.status as TaskStatus]} colorClass={taskStatusColors[task.status as TaskStatus]} />
      </div>
      <p className="text-sm text-slate-600 dark:text-slate-300 mb-3 line-clamp-2">{task.brief}</p>
      <div className="flex items-center justify-between text-xs text-slate-500 mb-2">
        <span className="flex items-center gap-1"><Clock className="w-3 h-3" />{task.deadline}</span>
        <span className="font-medium text-gray-600">{task.payment.toLocaleString()}₫</span>
      </div>
    ...
    <div className="space-y-4">
      ...
      <div className="card-base p-6">
        <div className="flex items-start justify-between mb-4">
          <div>
            <p className="text-xs text-brand-500 dark:text-brand-400 font-medium">{task.brandName}</p>
            <h3 className="text-xl font-bold text-slate-900 dark:text-white">{task.campaignName}</h3>
            <p className="text-sm text-slate-500 mt-0.5">{task.productName}</p>
          </div>
          <Badge label={taskStatusLabels[task.status as TaskStatus]} colorClass={taskStatusColors[task.status as TaskStatus]} />
        </div>
        ...
        {[
          ...
          <button key={tab.id} onClick={() => setActiveTab(tab.id as any)}
            className={`px-4 py-2 text-sm font-medium rounded-lg transition-colors ${
              activeTab === tab.id ? 'bg-white dark:bg-slate-600 text-slate-900 dark:text-white shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-500'
            }`}>
            {tab.label}
          </button>
        ))}
      ...
      {activeTab === 'detail' && (
        <div className="space-y-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200/80 dark:border-slate-700/60 shadow-soft p-5">
            <h4 className="text-sm font-semibold text-slate-900 dark:text-white mb-2 flex items-center gap-1.5"><FileText className="w-4 h-4" />Brief nội dung</h4>
            <p className="text-sm text-slate-600 dark:text-slate-300">{task.brief}</p>
          </div>
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200/80 dark:border-slate-700/60 shadow-soft p-5">
            <h4 className="text-sm font-semibold text-slate-900 dark:text-white mb-2 flex items-center gap-1.5"><CheckCircle2 className="w-4 h-4" />Yêu cầu nội dung</h4>
            <p className="text-sm text-slate-600 dark:text-slate-300">{task.contentRequirements}</p>
          </div>
        ...
        <div className="space-y-3">
          {task.drafts.length === 0 ? (
            <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200/80 dark:border-slate-700/60 shadow-soft p-8 text-center">
              ...
              <p className="text-sm text-slate-500">Chưa có bản nháp nào được gửi.</p>
              {canSubmitDraft && (
            ...
            [...task.drafts].reverse().map((draft, i) => (
              <div key={i} className={`rounded-2xl border overflow-hidden ${i === 0 && draft.status === 'draft_submitted' ? 'bg-amber-50/80 dark:bg-amber-900/20 border-amber-200 dark:border-amber-800/40' : 'bg-white dark:bg-slate-800 bg-slate-200 dark:border-slate-600'}`}>
                <div className="p-4">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-semibold text-slate-900 dark:text-white">Phiên bản {draft.version}</span>
                      <Badge label={draft.status === 'approved' ? 'Đã duyệt' : draft.status === 'draft_submitted' ? 'Chờ duyệt' : 'Cần sửa'}
                        colorClass={draft.status === 'approved' ? 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300' :
                          draft.status === 'draft_submitted' ? 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300' :
                          'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300'} />
                    </div>
                    <span className="text-xs text-slate-400">Nộp lúc {draft.submittedAt}</span>
                  </div>
                  {draft.image && (
                    <img src={draft.image} alt={`Draft v${draft.version}`} className="w-full h-48 object-cover rounded mb-3 cursor-pointer transition-opacity"
                      onClick={() => setShowPreviewDraft(i)} />
                  )}
                  <p className="text-sm text-slate-600 dark:text-slate-300 italic">"{draft.caption}"</p>
                  {draft.feedback && (
                    <div className="mt-3 p-3 bg-gray-50 dark:bg-gray-700 rounded">
                      <p className="text-xs text-gray-600 dark:text-gray-400 font-medium mb-1 flex items-center gap-1"><MessageCircle className="w-3.5 h-3.5" />Phản hồi từ Brand:</p>
                      <p className="text-xs text-gray-400 dark:text-gray-400">{draft.feedback}</p>
                    </div>
                  )}
                </div>
              </div>
        ...
        <div className="space-y-4">
          {!hasMetrics ? (
            <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200/80 dark:border-slate-700/60 shadow-soft p-8 text-center">
              ...
              <p className="text-sm text-slate-500">Chưa có dữ liệu performance. Gửi link bài đăng để bắt đầu theo dõi.</p>
              {canSubmitLink && (
            ...
            <>
              <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                {[
                  { label: 'Views', value: task.metrics.views >= 1000 ? `${Math.round(task.metrics.views/1000)}K` : task.metrics.views.toString(), color: 'text-slate-900' },
                  ...
                  <div key={s.label} className="bg-white dark:bg-slate-800 rounded-xl border bg-slate-200 dark:border-slate-600 p-3 text-center">
                    <p className={`text-lg font-bold ${s.color}`}>{s.value}</p>
                    <p className="text-xs text-slate-500">{s.label}</p>
                  </div>
                ))}
              </div>
              <div className="bg-brand-50/80 dark:bg-brand-900/20 rounded-2xl border border-brand-200 dark:border-brand-800/40 p-4 text-center">
                <p className="text-xs text-brand-500 dark:text-brand-400 mb-1">Engagement Rate</p>
                    ...
                    <tbody>
                      {task.metrics.snapshots.map((snap, i) => (
                        <tr key={i} className="border-b border-slate-50 dark:border-slate-700/50">
                          <td className="px-4 py-3 font-medium text-slate-900 dark:text-white">{snap.time}</td>
                          <td className="px-4 py-3 text-slate-700 dark:text-slate-200">{snap.views.toLocaleString()}</td>
                          <td className="px-4 py-3 text-slate-700 dark:text-slate-200">{snap.likes.toLocaleString()}</td>
                          <td className="px-4 py-3 text-slate-700 dark:text-slate-200">{snap.comments.toLocaleString()}</td>
                          <td className="px-4 py-3 text-slate-700 dark:text-slate-200">{snap.shares.toLocaleString()}</td>
                          <td className="px-4 py-3 text-slate-700 dark:text-slate-200">{snap.saves.toLocaleString()}</td>
                        </tr>
                      ))}
                    </tbody>
                    </table>
      ...
      <Modal isOpen={showPreviewDraft !== null} onClose={() => setShowPreviewDraft(null)} title={`Xem trước bản nháp v${showPreviewDraft !== null ? task.drafts[showPreviewDraft]?.version : ''}`} width="max-w-2xl">
        {showPreviewDraft !== null && task.drafts[showPreviewDraft] && (
          <div className="space-y-3">
            ...
            <p className="text-sm text-slate-600 dark:text-slate-300 italic">"{task.drafts[showPreviewDraft].caption}"</p>
          </div>
        )}
      </Modal>
    </div>
...
function StatCard({ label, value, suffix, accent }: { label: string; value: string; suffix?: string; accent?: string }) {
  return (
    <div className="bg-slate-50/50 dark:bg-slate-700/50 rounded-xl p-3 text-center">
      <p className="text-xs text-slate-500 mb-0.5">{label}</p>
      <p className={`text-base font-bold ${accent === 'teal' ? 'text-teal-600' : 'text-slate-900 dark:text-white'}`}>{value}{suffix}</p>
    </div>
  );
}
...
function ActionCard({ icon, color, title, desc, btnLabel, onClick }: {
  ...
  const colors = {
    amber: 'bg-amber-50/80 dark:bg-amber-900/20 border-amber-200 dark:border-amber-800/40 text-amber-600',
    teal: 'bg-brand-50/80 dark:bg-brand-900/20 border-brand-200 dark:border-brand-800/40 text-teal-600',
    blue: 'bg-blue-50/80 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800/40 text-blue-600',
  };
  return (
    <div className={`rounded border p-5 ${colors[color as keyof typeof colors]}`}>
      <div className="flex items-center gap-2 mb-2">
        <div className={`p-2 rounded bg-white/50 dark:bg-gray-800/50 ${colors[color as keyof typeof colors]}`}>{icon}</div>
        <div>
          ...
          <p className={`text-xs mt-0.5 ${color === 'amber' ? 'text-amber-600 dark:text-amber-400' : color === 'teal' ? 'text-brand-500 dark:text-brand-400' : 'text-blue-600 dark:text-blue-400'}`}>{desc}</p>
        </div>
      </div>
      <Button onClick={onClick} className={
        color === 'amber' ? 'bg-gray-600 hover:bg-gray-700' :
        color === 'teal' ? 'bg-gray-600 hover:bg-gray-700' :
...
function SubmitDraftForm({ task, onClose }: { task: typeof kolTasks[0]; onClose: () => void }) {
  ...
  if (step === 'done') {
    return (
      <div className="space-y-4 text-center py-4">
        ...
        <h3 className="text-lg font-bold text-slate-900 dark:text-white">Bản nháp đã được gửi!</h3>
        <p className="text-sm text-slate-500">Brand sẽ xem xét và phản hồi trong 24-48 giờ. Bạn sẽ nhận được thông báo khi có cập nhật mới.</p>
        <div className="p-4 bg-slate-50/50 dark:bg-slate-700/50 rounded-xl text-left space-y-2 text-sm">
          <div className="flex justify-between"><span className="text-slate-500">Chiến dịch</span><span className="font-medium text-slate-900 dark:text-white">{task.campaignName}</span></div>
          <div className="flex justify-between"><span className="text-slate-500">Sản phẩm</span><span className="font-medium text-slate-900 dark:text-white">{task.productName}</span></div>
          <div className="flex justify-between"><span className="text-slate-500">File đính kèm</span><span className="font-medium text-slate-900 dark:text-white">{uploadedFiles.length} file(s)</span></div>
          <div className="flex justify-between"><span className="text-slate-500">Caption</span><span className="font-medium text-teal-600 truncate max-w-[180px]">{caption}</span></div>
        </div>
  ...
  if (step === 'preview') {
    return (
      <div className="space-y-4">
        <div className="p-3 bg-brand-50/80 dark:bg-brand-900/20 rounded-xl border border-brand-200 dark:border-brand-800/40 text-sm text-teal-700 dark:text-teal-300">
        ...
        <div className="rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
          ...
          <div className="p-4 bg-white dark:bg-gray-800">
            <div className="flex items-center gap-2 mb-3">
              <Avatar initials="LB" size="sm" image={getKolImage('LB')} />
              <div>
                <p className="text-sm font-semibold text-slate-900 dark:text-white">{currentKOL.name}</p>
                <p className="text-xs text-slate-400">@{currentKOL.handle}</p>
              </div>
            </div>
            <p className="text-sm text-slate-700 dark:text-slate-200 mb-2 whitespace-pre-wrap">{captionPreview}</p>
            <p className="text-xs text-slate-400">{hashtag}</p>
          </div>
        </div>
        <div className="space-y-2">
          <p className="text-xs font-semibold text-slate-500 uppercase">File đính kèm ({uploadedFiles.length})</p>
          {uploadedFiles.map((f, i) => (
            <div key={i} className="flex items-center gap-2 p-2 bg-slate-50/50 dark:bg-slate-700/50 rounded-xl">
              <FileText className="w-4 h-4 text-teal-500 flex-shrink-0" />
              <span className="text-sm text-slate-700 dark:text-slate-200 flex-1 truncate">{f}</span>
              <span className="text-xs text-slate-400">{(12 + i * 5).toFixed(1)} MB</span>
            </div>
          ))}
        </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        <label className="block text-sm font-medium text-slate-500 dark:text-slate-300 mb-1.5">File nội dung *</label>
        <div className="border-2 border-dashed bg-slate-200 dark:border-slate-600 rounded-xl p-6 text-center cursor-pointer hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors" onClick={addFile}>
          {uploadedFiles.length > 0 ? (
            <div className="space-y-3">
              {uploadedFiles.map((f, i) => (
                <div key={i} className="flex items-center justify-center gap-2 text-teal-600 bg-brand-50/80 dark:bg-brand-900/20 rounded-xl p-2">
                  <FileText className="w-4 h-4" />
                  <span className="text-sm font-medium">{f}</span>
                  <button onClick={(e) => { e.stopPropagation(); removeFile(i); }} className="text-gray-400 hover:text-gray-600 ml-1"><X className="w-3 h-3" /></button>
                </div>
              ))}
              <p className="text-xs text-slate-400">Nhấn để thêm file khác</p>
            </div>
          ) : (
            <div>
              <Upload className="w-10 h-10 text-slate-400 mx-auto mb-2" />
              <p className="text-sm text-slate-500">Kéo thả hoặc nhấn để tải lên video/hình ảnh</p>
              <p className="text-xs text-slate-400 mt-1">MP4, MOV, JPG tối đa 500MB</p>
            </div>
          )}
        </div>
      </div>
      <div>
        <label className="block text-sm font-medium text-slate-500 dark:text-slate-300 mb-1.5">Caption / Mô tả *</label>
        <textarea rows={4} value={caption} onChange={e => setCaption(e.target.value)}
          className="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-700 text-slate-900 dark:text-white text-sm resize-none focus:ring-2 focus:ring-teal-500 outline-none"
          placeholder="Viết caption bạn dự định đăng kèm nội dung..." />
        <p className="text-xs text-slate-400 mt-1">{caption.length}/2,200 ký tự</p>
      </div>
      <div>
        <label className="block text-sm font-medium text-slate-500 dark:text-slate-300 mb-1.5">Hashtag gợi ý</label>
        <input type="text" value={hashtag} onChange={e => setHashtag(e.target.value)}
          className="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-700 text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-teal-500 outline-none" />
      </div>
      <div className="flex gap-3">
...
function SubmitLinkForm({ task, onClose }: { task: typeof kolTasks[0]; onClose: () => void }) {
  ...
  if (step === 'done') {
    return (
      <div className="space-y-4 text-center py-4">
        ...
        <h3 className="text-lg font-bold text-slate-900 dark:text-white">Link đã được gửi!</h3>
        <p className="text-sm text-slate-500">Brand đã nhận được link bài đăng. Metrics sẽ được theo dõi tự động. Bạn sẽ nhận được thanh toán sau khi hoàn tất.</p>
        <div className="p-4 bg-slate-50/50 dark:bg-slate-700/50 rounded-xl text-left space-y-2 text-sm">
          <div className="flex justify-between"><span className="text-slate-500">Chiến dịch</span><span className="font-medium text-slate-900 dark:text-white">{task.campaignName}</span></div>
          <div className="flex justify-between"><span className="text-slate-500">Link</span><span className="font-medium text-teal-600 truncate max-w-[180px]">{postUrl}</span></div>
          <div className="flex justify-between"><span className="text-slate-500">Thời gian đăng</span><span className="font-medium text-slate-900 dark:text-white">{publishedAt}</span></div>
          <div className="flex justify-between"><span className="text-slate-500">Ảnh chụp</span><span className="font-medium text-emerald-600">✓ Đã tải lên</span></div>
        </div>
        <Button onClick={onClose} className="w-full">Đóng</Button>
      </div>
    );
  ...
  if (step === 'preview') {
    return (
      <div className="space-y-4">
        ...
        <div className="rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
          ...
          <div className="p-4 bg-white dark:bg-gray-800">
            <div className="flex items-center gap-2 mb-3">
              ...
              <div>
                <p className="text-sm font-semibold text-slate-900 dark:text-white">{currentKOL.name}</p>
                <p className="text-xs text-slate-400">{publishedAt}</p>
              </div>
            </div>
            <p className="text-sm text-slate-700 dark:text-slate-200 mb-2 whitespace-pre-wrap">{captionPreview}</p>
            <div className="flex items-center gap-3 text-slate-400 text-xs mt-2 pt-2 border-t border-slate-100/60">
        ...
        <div className="p-3 bg-slate-50/50 dark:bg-slate-700/50 rounded-xl space-y-1 text-sm">
          <div className="flex justify-between"><span className="text-slate-500">Link bài đăng</span><span className="text-teal-600 font-medium truncate max-w-[180px]">{postUrl}</span></div>
          <div className="flex justify-between"><span className="text-slate-500">Thời gian đăng</span><span className="text-slate-900 dark:text-white">{publishedAt}</span></div>
          <div className="flex justify-between"><span className="text-slate-500">Ảnh chụp</span><span className="text-emerald-600 font-medium">✓ Đã tải lên</span></div>
        </div>
        <div className="flex gap-3">
  ...
  return (
    <div className="space-y-4">
      <div className="p-3 bg-brand-50/80 dark:bg-brand-900/20 rounded-xl border border-brand-200 dark:border-brand-800/40 text-xs text-teal-700 dark:text-teal-300">
        Gửi link bài đăng đã xuất bản kèm ảnh chụp minh chứng. Brand sẽ xác nhận và bắt đầu theo dõi metrics.
      </div>
      <div>
        <label className="block text-sm font-medium text-slate-500 dark:text-slate-300 mb-1.5">Link bài đăng đã xuất bản *</label>
        <input type="url" value={postUrl} onChange={e => setPostUrl(e.target.value)}
          className="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-700 text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-teal-500 outline-none"
          placeholder="https://tiktok.com/@.../video/..." />
      </div>
      <div>
        <label className="block text-sm font-medium text-slate-500 dark:text-slate-300 mb-1.5">Thời gian đăng *</label>
        <input type="datetime-local" value={publishedAt} onChange={e => setPublishedAt(e.target.value)}
          className="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-700 text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-teal-500 outline-none" />
      </div>
      <div>
        <label className="block text-sm font-medium text-slate-500 dark:text-slate-300 mb-1.5">Ảnh chụp bài đăng *</label>
        <div className="border-2 border-dashed bg-slate-200 dark:border-slate-600 rounded-xl p-6 text-center cursor-pointer hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors"
          ...
          {screenshot ? (
            ...
            <div>
              ...
              <p className="text-sm text-slate-500">Tải lên ảnh chụp bài đăng từ nền tảng</p>
...
function SubmitMetricsForm({ task, onClose }: { task: typeof kolTasks[0]; onClose: () => void }) {
  ...
  if (step === 'done') {
    return (
      <div className="space-y-4 text-center py-4">
        ...
        <h3 className="text-lg font-bold text-slate-900 dark:text-white">Metrics đã được gửi!</h3>
        <p className="text-sm text-slate-500">Brand sẽ xác nhận số liệu. Thanh toán sẽ được xử lý sau khi metrics được duyệt. Thứ hạng của bạn sẽ được cập nhật.</p>
        <div className="p-4 bg-slate-50/50 dark:bg-slate-700/50 rounded-xl text-left space-y-2 text-sm">
          <div className="flex justify-between"><span className="text-slate-500">Chiến dịch</span><span className="font-medium text-slate-900 dark:text-white">{task.campaignName}</span></div>
          <div className="flex justify-between"><span className="text-slate-500">Views</span><span className="font-bold text-slate-900 dark:text-white">{v.toLocaleString()}</span></div>
          <div className="flex justify-between"><span className="text-slate-500">Engagement Rate</span><span className="font-bold text-blue-600">{er}%</span></div>
          <div className="flex justify-between"><span className="text-slate-500">Thời điểm</span><span className="font-medium text-slate-900 dark:text-white">{snapshotLabel}</span></div>
        </div>
        <Button onClick={onClose} className="w-full">Đóng</Button>
      </div>
    );
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4 text-center">
            <p className="text-xs text-slate-500 mb-1">Views của bạn</p>
            <p className="text-2xl font-bold text-slate-900 dark:text-white">{v >= 1000 ? `${Math.round(v/1000)}K` : v}</p>
            ...
            <p className="text-xs text-slate-400 mt-1">{viewsPct}% target ({targetViews >= 1000 ? Math.round(targetViews/1000)+'K' : targetViews})</p>
          </div>
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4 text-center">
            ...
            <p className="text-xs text-slate-400 mt-1">{erPct}% target ({targetER}%)</p>
          </div>
        </div>
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
          <div className="overflow-x-auto">
            ...
            <tbody>
              {[
                { label: 'Views', value: v, target: targetViews, color: 'text-slate-900' },
                ...
                <tr key={i} className="border-b border-slate-50 dark:border-slate-700/50">
                  ...
                  <td className="px-4 py-2.5 text-slate-400">{row.target}</td>
                  <td className="px-4 py-2.5">
                    {row.isString ? (
                      <Badge label={parseFloat(er) >= targetER ? '✓ Đạt' : '⚠ Thấp'} colorClass={parseFloat(er) >= targetER ? 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300' : 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300'} />
                    ) : (
                      <Badge label={row.value === '-' ? '-' : (row.value as number) >= (row.target as number) ? '✓ Đạt' : '⚠ Thấp'}
                        colorClass={row.value === '-' || (row.value as number) >= (row.target as number) ? 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300' : 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300'} />
                    )}
                  </td>
                </tr>
              ))}
        ...
        <div className="bg-slate-50/50 dark:bg-slate-700/50 rounded-xl p-3 text-center">
          <div className="w-32 h-32 bg-slate-200 dark:bg-slate-600 rounded-lg flex items-center justify-center mx-auto mb-1">
            <Camera className="w-10 h-10 text-slate-400" />
          </div>
          <p className="text-xs text-slate-400">screenshot_20260602.png — Đã tải lên</p>
        </div>
        <div className="flex gap-3">
          <Button variant="secondary" onClick={() => setStep('form')} className="flex-1">← Sửa lại</Button>
  ...
  return (
    <div className="space-y-4">
      <div className="p-3 bg-blue-50/80 dark:bg-blue-900/20 rounded-xl border border-blue-200 dark:border-blue-800/40 text-xs text-blue-700 dark:text-blue-300 flex items-center justify-between">
        ...
        <select value={snapshotLabel} onChange={e => setSnapshotLabel(e.target.value)}
          className="ml-2 px-2 py-1 rounded border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-700 text-gray-600 dark:text-gray-300 text-xs">
          {['24h', '72h', '7 ngày', 'Final'].map(l => <option key={l}>{l}</option>)}
        </select>
      </div>
      <div className="grid grid-cols-2 gap-3">
        {[
          { label: 'Views *', value: views, setter: setViews, placeholder: 'VD: 50000', icon: <Eye className="w-4 h-4 text-slate-400" /> },
          ...
          <div key={f.label} className="relative">
            ...
            <input type="number" value={f.value} onChange={e => f.setter(e.target.value)}
              className="w-full pl-8 pr-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-700 text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-teal-500 outline-none"
      ...
      <div className="relative">
        <label className="block text-xs text-slate-500 mb-1">Saves</label>
        <input type="number" value={saves} onChange={e => setSaves(e.target.value)}
          className="w-full pl-8 pr-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-700 text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-teal-500 outline-none"
          placeholder="VD: 150" />
        <div className="absolute bottom-2.5 left-2.5"><Bookmark className="w-4 h-4 text-gray-600" /></div>
      </div>
      <div className="p-4 bg-brand-50/80 dark:bg-brand-900/20 rounded-xl text-center">
        <p className="text-xs text-brand-500 dark:text-brand-400 mb-1">Engagement Rate tự động tính</p>
        <p className="text-3xl font-bold text-teal-600">{er}%</p>
        <p className="text-xs text-slate-400 mt-1">Target: {task.kpiTarget.engagementRate}%</p>
      </div>
      <div>
        <label className="block text-sm font-medium text-slate-500 dark:text-slate-300 mb-1.5">Ảnh chụp minh chứng *</label>
        <div className="border-2 border-dashed bg-slate-200 dark:border-slate-600 rounded-xl p-6 text-center cursor-pointer hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors"
          onClick={() => setScreenshot(true)}>
          {screenshot ? (
            <div className="flex items-center justify-center gap-2 text-gray-600">
            ...
            <div>
              <Camera className="w-8 h-8 text-slate-400 mx-auto mb-2" />
              <p className="text-sm text-slate-500">Tải lên ảnh chụp màn hình từ nền tảng</p>
              <p className="text-xs text-slate-400 mt-1">JPG, PNG tối đa 10MB</p>
            </div>
          )}
        </div>
...
function WalletView() {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <h3 className="text-xl font-bold text-slate-900 dark:text-white">Ví & Thanh toán</h3>

      {/* Summary */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className=" rounded border border-gray-200 dark:border-gray-700 p-5">
          <p className="text-sm text-gray-600 dark:text-gray-400 mb-1">Tổng đã kiếm được</p>
          <p className="text-2xl font-bold text-gray-600">{(currentKOL.totalEarned / 1000000).toFixed(1)}M ₫</p>
        </div>
        <div className="bg-gradient-to-br from-amber-500/10 to-amber-600/5 rounded-2xl border border-amber-200/60 dark:border-amber-800/40 p-5">
          ...
          <p className="text-xs text-slate-400 mt-1">{pending.length} khoản</p>
        </div>
        <div className="bg-gradient-to-br from-teal-500/10 to-teal-600/5 rounded-2xl border border-teal-200/60 dark:border-teal-800/40 p-5">
          <p className="text-sm text-brand-500 dark:text-brand-400 mb-1">Đã thanh toán</p>
          <p className="text-2xl font-bold text-teal-600">{(paid.reduce((s, p) => s + p.paidAmount, 0) / 1000000).toFixed(1)}M ₫</p>
          <p className="text-xs text-slate-400 mt-1">{paid.length} khoản</p>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200/80 dark:border-slate-700/60 shadow-soft overflow-hidden">
        <div className="p-4 border-b border-slate-100/60 flex items-center justify-between">
          <h4 className="text-sm font-semibold text-slate-900 dark:text-white">Lịch sử thanh toán</h4>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[700px]">
            ...
            <tbody>
              {kolPayments.map(p => (
                <tr key={p.id} className="border-b border-slate-50 dark:border-slate-700/50 hover:bg-slate-50/70 dark:hover:bg-slate-700/30">
                  ...
                  <td className="px-6 py-4">
                    <Badge label={paymentStatusLabels[p.status]} colorClass={
                      p.status === 'paid' ? 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300' :
                      'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300'
                    } />
                  </td>
      ...
      {/* Bank info */}
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200/80 dark:border-slate-700/60 shadow-soft p-5">
        <h4 className="text-sm font-semibold text-slate-900 dark:text-white mb-3 flex items-center gap-1.5"><Wallet className="w-4 h-4" />Thông tin nhận tiền</h4>
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div className="flex justify-between"><span className="text-slate-500">Ngân hàng</span><span className="font-medium text-slate-900 dark:text-white">{currentKOL.bankName}</span></div>
          <div className="flex justify-between"><span className="text-slate-500">Số tài khoản</span><span className="font-medium text-slate-900 dark:text-white">{currentKOL.bankAccount}</span></div>
          <div className="flex justify-between"><span className="text-slate-500">Tên tài khoản</span><span className="font-medium text-slate-900 dark:text-white">{currentKOL.bankHolder}</span></div>
          <div className="flex justify-between"><span className="text-slate-500">QR thanh toán</span>
            <Badge label={currentKOL.qrUploaded ? 'Đã tải' : 'Chưa tải'} colorClass={currentKOL.qrUploaded ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100'} />
          </div>
        </div>
      </div>
      ...
      <Modal isOpen={showWithdraw} onClose={() => setShowWithdraw(false)} title="Rút tiền" width="max-w-md">
        <div className="space-y-4">
          <div className="p-4 bg-amber-50/80 dark:bg-amber-900/20 rounded-xl border border-amber-200 dark:border-amber-800/40 text-sm text-amber-700 dark:text-amber-300">
          ...
          <div>
            <label className="block text-sm font-medium text-slate-500 dark:text-slate-300 mb-1.5">Số tiền muốn rút (₫)</label>
            <input type="number" defaultValue={currentKOL.pendingPayment} className="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-700 text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-teal-500 outline-none" />
          </div>
          <div className="flex justify-end gap-3">
...
function ProfileView({ onBack }: { onBack: () => void }) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="sm" onClick={onBack}>← Quay lại</Button>
        <h3 className="text-xl font-bold text-slate-900 dark:text-white">Hồ sơ</h3>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="card-base p-6">
          <div className="flex flex-col items-center mb-6">
            <div className="relative mb-4">
              <Avatar initials={currentKOL.avatar} size="lg" image={getKolImage(currentKOL.avatar)} />
            </div>
            <h3 className="text-lg font-bold text-slate-900 dark:text-white">{profileData.name}</h3>
            <p className="text-sm text-slate-500">{currentKOL.handle} • {currentKOL.platform}</p>
            <div className="flex gap-2 mt-2">
              <Badge label={profileData.contentCategory} colorClass="bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300" />
              <Badge label={`#${currentKOL.currentRank} Top KOL`} colorClass="bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300" />
            </div>
          </div>
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={item.label} className="flex items-center justify-between p-3 bg-slate-50/50 dark:bg-slate-700/50 rounded-xl">
                <div className="flex items-center gap-2 text-slate-500">{item.icon}<span className="text-sm">{item.label}</span></div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="flex justify-between items-center">
            <h4 className="text-sm font-semibold text-slate-900 dark:text-white">Thông tin cá nhân</h4>
          ...
          {/* Bio */}
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200/80 dark:border-slate-700/60 shadow-soft p-5">
            <h4 className="text-xs font-semibold text-slate-500 uppercase mb-2">Giới thiệu</h4>
            {isEditing ? (
              ...
              <p className="text-sm text-slate-600 dark:text-slate-300">{profileData.bio}</p>
            )}
          </div>

          {/* Category & Platform */}
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200/80 dark:border-slate-700/60 shadow-soft p-5">
            <h4 className="text-xs font-semibold text-slate-500 uppercase mb-3">Thông tin nền tảng</h4>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs text-slate-500 mb-1">Lĩnh vực nội dung</label>
                {isEditing ? (
                  ...
                  <p className="text-sm font-medium text-slate-900 dark:text-white">{profileData.contentCategory}</p>
                )}
              </div>
              <div>
                <label className="block text-xs text-slate-500 mb-1">Nền tảng chính</label>
                {isEditing ? (
                  ...
                  <p className="text-sm font-medium text-slate-900 dark:text-white">{profileData.platform}</p>
                )}
              </div>
            </div>
          </div>
          ...
          {/* Social Links */}
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200/80 dark:border-slate-700/60 shadow-soft p-5">
            <h4 className="text-xs font-semibold text-slate-500 uppercase mb-3">Liên kết mạng xã hội</h4>
            <div className="space-y-3">
              <div>
                <label className="block text-xs text-slate-500 mb-1">Email liên hệ</label>
                {isEditing ? (
                  <input type="email" value={profileData.email} onChange={e => setProfileData({ ...profileData, email: e.target.value })}
                    className="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-700 text-slate-900 dark:text-white text-sm" placeholder="email@example.com" />
                ) : (
                  <a href={`mailto:${profileData.email}`} className="text-sm text-gray-600 hover:underline flex items-center gap-1">
              ...
              {[
                ...
                <div key={link.label}>
                  <label className="block text-xs text-slate-500 mb-1">{link.label}</label>
                  {isEditing ? (
                    <input type="url" value={link.value} onChange={e => setProfileData({ ...profileData, [link.label.toLowerCase()]: e.target.value })}
                      className="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-700 text-slate-900 dark:text-white text-sm" />
                  ) : (
                    <a href={link.value} className="text-sm text-gray-600 hover:underline flex items-center gap-1">
          ...
          {/* QR Payment */}
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200/80 dark:border-slate-700/60 shadow-soft p-5">
            <h4 className="text-xs font-semibold text-slate-500 uppercase mb-3 flex items-center gap-1.5"><QrCode className="w-4 h-4" />Mã QR thanh toán</h4>
            <div className="border-2 border-dashed bg-slate-200 dark:border-slate-600 rounded-xl p-6 text-center cursor-pointer hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors"
              onClick={() => setQrUploaded(!qrUploaded)}>
              {qrUploaded ? (
                <div>
                  <div className="w-20 h-20 bg-slate-100/60 dark:bg-slate-700 rounded-lg flex items-center justify-center mx-auto mb-2">
                    <QrCode className="w-12 h-12 text-teal-500" />
                  </div>
                  <p className="text-sm text-teal-600 font-medium">QR đã được tải lên</p>
                  <p className="text-xs text-slate-400 mt-1">Nhấn để thay thế</p>
                </div>
              ) : (
                <div>
                  <Upload className="w-8 h-8 text-slate-400 mx-auto mb-2" />
                  <p className="text-sm text-slate-500">Tải lên mã QR thanh toán</p>
