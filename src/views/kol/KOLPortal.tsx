    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button className="relative p-2 rounded hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-gray-50 dark:bg-gray-700 rounded p-4 text-center border border-gray-200 dark:border-gray-700">
          <p className="text-xs text-gray-600 dark:text-gray-400 mb-1">Tổng đã nhận</p>
          <p className="text-2xl font-bold text-gray-600">{(totalPaid / 1000000).toFixed(1)}M ₫</p>
          <p className="text-xs text-gray-400 mt-1">{paidPayments.length} khoản</p>
        </div>
        <div className="bg-gray-50 dark:bg-gray-700 rounded p-4 text-center border border-gray-200 dark:border-gray-700">
          <p className="text-xs text-gray-600 dark:text-gray-400 mb-1">Tháng này</p>
          <p className="text-2xl font-bold text-gray-600">{(currentMonth.amount / 1000000).toFixed(1)}M ₫</p>
          ...
          {monthlyData.map((m) => {
            ...
            return (
              <div key={m.month} className="flex-1 flex flex-col items-center gap-1">
                ...
                <div className="w-full rounded-t-lg bg-gray-500 dark:bg-gray-600 transition-colors" style={{ height: `${Math.max(pct, 4)}%` }} />
...
function KPIPopupPendingPayments() {
  ...
  return (
    <div className="space-y-4">
      <div className="bg-gray-50 dark:bg-gray-700 rounded p-4 border border-gray-200 dark:border-gray-700 text-center">
        <p className="text-xs text-gray-600 dark:text-gray-400 mb-1">Tổng đang chờ</p>
        <p className="text-3xl font-bold text-gray-600">{(currentKOL.pendingPayment / 1000000).toFixed(1)}M ₫</p>
          ...
          return (
            <div key={p.id} className="rounded border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700 p-4 space-y-2">
              ...
              <div className="flex items-center gap-4 text-xs text-gray-500">
                <span className="flex items-center gap-1"><CheckCircle2 className="w-3 h-3 text-gray-400" />Đã gửi metrics</span>
                <span className="flex items-center gap-1"><Clock className="w-3 h-3 text-gray-400" />Chờ Brand duyệt</span>
...
function KPIPopupRanking() {
  const rankings = [
    { rank: 1, name: 'Linh Beauty', platform: 'TikTok', followers: '1.2M', er: '6.8%', score: 98, trend: '+2', color: 'bg-gray-500', badge: '🏆' },
    { rank: 2, name: 'Minh Tech Review', platform: 'YouTube', followers: '850K', er: '5.2%', score: 91, trend: '-1', color: 'bg-gray-400', badge: '' },
    { rank: 3, name: 'Foodie Hà Nội', platform: 'TikTok', followers: '2.1M', er: '4.1%', score: 89, trend: '+1', color: 'bg-gray-700', badge: '' },
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white dark:bg-gray-800 rounded border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div className="overflow-x-auto">
          ...
          <tbody>
            {rankings.map((r) => (
              <tr key={r.rank} className={`border-b border-gray-100 dark:border-gray-700 ${r.name === currentKOL.name ? 'bg-gray-50 dark:bg-gray-700' : ''}`}>
                ...
                <td className="px-3 py-2.5 text-xs font-medium text-gray-600">{r.er}</td>
                <td className="px-3 py-2.5 text-sm font-bold text-gray-900 dark:text-white">{r.score}</td>
                <td className="px-3 py-2.5 text-xs font-medium">
                  {r.trend.startsWith('+') && <span className="text-gray-600">↑ {r.trend}</span>}
                  {r.trend.startsWith('-') && <span className="text-gray-400">↓ {r.trend}</span>}
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3 max-h-[350px] overflow-y-auto">
        {completedTasks.map(task => (
          <div key={task.id} className="bg-white dark:bg-gray-800 rounded border border-gray-200 dark:border-gray-700 p-4 space-y-2">
            <div className="flex items-center justify-between">
              <div>
            ...
            {task.metrics.views > 0 && (
              <div className="grid grid-cols-5 gap-2 pt-2 border-t border-gray-100 dark:border-gray-700">
                {[
...
function KPICard({ label, value, suffix, icon, accent = 'bg-gray-50', onClick }: {
  ...
  return (
    <div onClick={onClick}
      className={`${accent} rounded p-4 border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 ${onClick ? 'cursor-pointer' : ''}`}>
...
function DashboardView({ navigateTo, onSelectTask }: {
  ...
  return (
    <div className="space-y-6">
      {/* Quick Nav */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <QuickNavCard icon={<Search className="w-5 h-5" />} label="Khám phá chiến dịch" count={exploreCampaigns.length} color="text-gray-600" bg="bg-gray-50 dark:bg-gray-700"
          onClick={() => navigateTo('explore')} />
        <QuickNavCard icon={<FileText className="w-5 h-5" />} label="Nhiệm vụ của tôi" count={kolTasks.length} color="text-gray-600" bg="bg-gray-50 dark:bg-gray-700"
          onClick={() => navigateTo('tasks')} />
        <QuickNavCard icon={<Wallet className="w-5 h-5" />} label="Ví & Thanh toán" count={kolPayments.filter(p => p.status === 'pending').length} color="text-gray-600" bg="bg-gray-50 dark:bg-gray-700"
          onClick={() => navigateTo('wallet')} />
        <QuickNavCard icon={<Settings className="w-5 h-5" />} label="Cài đặt" count={0} color="text-gray-500" bg="bg-gray-50 dark:bg-gray-700"
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-base font-semibold text-gray-900 dark:text-white flex items-center gap-2">
              <AlertTriangle className="w-4 h-4 text-gray-400" />Cần xử lý ngay
            </h3>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-base font-semibold text-gray-900 dark:text-white flex items-center gap-2">
            <Star className="w-4 h-4 text-gray-400" />Chiến dịch mới dành cho bạn
          </h3>
      ...
      {kolTasks.filter(t => t.metrics.views > 0).length > 0 && (
        <div>
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-base font-semibold text-gray-900 dark:text-white flex items-center gap-2">
              <TrendingUp className="w-4 h-4 text-gray-400" />Performance nổi bật
            </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {kolTasks.filter(t => t.metrics.views > 0).slice(0, 3).map(task => (
              <div key={task.id} className="bg-white dark:bg-gray-800 rounded border border-gray-200 dark:border-gray-700 p-4">
                ...
                <div className="grid grid-cols-3 gap-2 text-center">
                  ...
                  <div>
                    <p className="text-base font-bold text-gray-600">{task.metrics.engagementRate}%</p>
                    <p className="text-[10px] text-gray-500">ER</p>
                  </div>
                  <div>
                    <p className="text-base font-bold text-gray-600">{task.metrics.likes >= 1000 ? `${Math.round(task.metrics.likes/1000)}K` : task.metrics.likes}</p>
...
function QuickNavCard({ icon, label, count, color, bg, onClick }: {
  ...
  return (
    <button onClick={onClick}
      className={`${bg} rounded p-4 border border-gray-200 dark:border-gray-700 text-left transition-colors flex items-center gap-3`}>
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {exploreCampaigns.map(c => (
          <div key={c.id} className="bg-white dark:bg-gray-800 rounded border border-gray-200 dark:border-gray-700 overflow-hidden">
            <img src={c.productImage} alt={c.productName} className="w-full h-36 object-cover" />
            <div className="p-4 space-y-2">
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-xs text-gray-600 dark:text-gray-400 font-medium">{c.brandName}</p>
              ...
              <div className="flex items-center justify-between pt-1 border-t border-gray-100 dark:border-gray-700">
                <p className="text-sm font-bold text-gray-600">{c.reward.toLocaleString()}₫</p>
...
function ExploreCard({ campaign }: { campaign: typeof exploreCampaigns[0] }) {
  return (
    <div className="bg-white dark:bg-gray-800 rounded border border-gray-200 dark:border-gray-700 overflow-hidden flex">
      <img src={campaign.productImage} alt={campaign.productName} className="w-24 h-24 object-cover flex-shrink-0" />
      <div className="p-3 flex-1 min-w-0">
        <p className="text-xs text-gray-600 font-medium">{campaign.brandName}</p>
...
function CampaignDetailModal({ campaignId, onClose }: {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-start justify-between">
        <div>
          <p className="text-sm font-semibold text-gray-600 dark:text-gray-400">{campaign.brandName}</p>
      ...
      <div className="space-y-3">
        <div className="bg-gray-50 dark:bg-gray-700 rounded p-4">
          <h4 className="text-sm font-semibold text-gray-600 dark:text-gray-300 mb-1.5 flex items-center gap-1.5"><FileText className="w-4 h-4" />Brief nội dung</h4>
      ...
      {!registered ? (
        ...
        <div className="space-y-3">
          <div className="p-3 bg-gray-50 dark:bg-gray-700 rounded border border-gray-200 dark:border-gray-700 text-sm text-gray-600 dark:text-gray-400">
        ...
        {[
          ...
          <button key={f.id} onClick={() => setFilter(f.id as any)}
            className={`px-3 py-1.5 rounded text-sm font-medium transition-colors ${
              filter === f.id ? 'bg-gray-600 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700'
            }`}>
            {f.label} ({f.count})
          </button>
        ))}
...
function TaskCard({ task, onClick }: { task: typeof kolTasks[0]; onClick: () => void }) {
  ...
  return (
    <div onClick={onClick}
      className="bg-white dark:bg-gray-800 rounded border border-gray-200 dark:border-gray-700 p-5 cursor-pointer  transition-colors">
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white dark:bg-gray-800 rounded border border-gray-200 dark:border-gray-700 p-6">
        <div className="flex items-start justify-between mb-4">
          <div>
            <p className="text-xs text-gray-600 dark:text-gray-400 font-medium">{task.brandName}</p>
        ...
        {[
          ...
          <button key={tab.id} onClick={() => setActiveTab(tab.id as any)}
            className={`px-4 py-2 text-sm font-medium rounded transition-colors ${
              activeTab === tab.id ? 'bg-white dark:bg-gray-700 text-gray-900 dark:text-white' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700'
            }`}>
            {tab.label}
          </button>
        ))}
        ...
        <div className="space-y-3">
          {task.drafts.length === 0 ? (
            ...
            [...task.drafts].reverse().map((draft, i) => (
              <div key={i} className={`rounded border overflow-hidden ${i === 0 && draft.status === 'draft_submitted' ? 'bg-gray-50 dark:bg-gray-700 border-gray-200 dark:border-gray-700' : 'bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700'}`}>
                <div className="p-4">
                  <div className="flex items-center justify-between mb-3">
            ...
            <>
              <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                {[
                  ...
                  { label: 'Likes', value: task.metrics.likes >= 1000 ? `${Math.round(task.metrics.likes/1000)}K` : task.metrics.likes.toString(), color: 'text-gray-600' },
                  { label: 'Comments', value: task.metrics.comments.toString(), color: 'text-gray-600' },
                  { label: 'Shares', value: task.metrics.shares >= 1000 ? `${Math.round(task.metrics.shares/1000)}K` : task.metrics.shares.toString(), color: 'text-gray-600' },
                  { label: 'Saves', value: task.metrics.saves >= 1000 ? `${Math.round(task.metrics.saves/1000)}K` : task.metrics.saves.toString(), color: 'text-gray-600' },
                ].map(s => (
              ...
              <div className="bg-gray-50 dark:bg-gray-700 rounded border border-gray-200 dark:border-gray-700 p-4 text-center">
                <p className="text-xs text-gray-600 dark:text-gray-400 mb-1">Engagement Rate</p>
                <p className="text-3xl font-bold text-gray-600">{task.metrics.engagementRate}%</p>
                <p className={`text-xs mt-1 ${task.metrics.brandConfirmed ? 'text-gray-600' : 'text-gray-600'}`}>
      ...
      <Modal isOpen={showPreviewDraft !== null} onClose={() => setShowPreviewDraft(null)} title={`Xem trước bản nháp v${showPreviewDraft !== null ? task.drafts[showPreviewDraft]?.version : ''}`} width="max-w-2xl">
        {showPreviewDraft !== null && task.drafts[showPreviewDraft] && (
          <div className="space-y-3">
            <img src={task.drafts[showPreviewDraft].image} alt="Preview" className="w-full rounded" />
...
function StatCard({ label, value, suffix, accent }: { label: string; value: string; suffix?: string; accent?: string }) {
  return (
    <div className="bg-gray-50 dark:bg-gray-700 rounded p-3 text-center">
      ...
      <p className={`text-base font-bold ${accent === 'teal' ? 'text-gray-600' : 'text-gray-900 dark:text-white'}`}>{value}{suffix}</p>
    </div>
  );
}
...
function ActionCard({ icon, color, title, desc, btnLabel, onClick }: {
  ...
  const colors = {
    amber: 'bg-gray-50 dark:bg-gray-700 border-gray-200 dark:border-gray-700 text-gray-600',
    teal: 'bg-gray-50 dark:bg-gray-700 border-gray-200 dark:border-gray-700 text-gray-600',
    blue: 'bg-gray-50 dark:bg-gray-700 border-gray-200 dark:border-gray-700 text-gray-600',
  };
  return (
    <div className={`rounded border p-5 ${colors[color as keyof typeof colors]}`}>
      <div className="flex items-center gap-2 mb-2">
        ...
        <div>
          <p className={`text-sm font-semibold ${color === 'amber' ? 'text-gray-600 dark:text-gray-400' : color === 'teal' ? 'text-gray-600 dark:text-gray-400' : 'text-gray-600 dark:text-gray-400'}`}>{title}</p>
          <p className={`text-xs mt-0.5 ${color === 'amber' ? 'text-gray-600 dark:text-gray-400' : color === 'teal' ? 'text-gray-600 dark:text-gray-400' : 'text-gray-600 dark:text-gray-400'}`}>{desc}</p>
        </div>
      </div>
...
function SubmitDraftForm({ task, onClose }: { task: typeof kolTasks[0]; onClose: () => void }) {
  ...
  if (step === 'done') {
    return (
      <div className="space-y-4 text-center py-4">
        ...
        <div className="p-4 bg-gray-50 dark:bg-gray-700 rounded text-left space-y-2 text-sm">
          ...
          <div className="flex justify-between"><span className="text-gray-500">Caption</span><span className="font-medium text-gray-600 truncate max-w-[180px]">{caption}</span></div>
        </div>
  ...
  return (
    <div className="space-y-4">
      <div className="p-3 bg-gray-50 dark:bg-gray-700 rounded border border-gray-200 dark:border-gray-700 text-xs text-gray-600 dark:text-gray-300">
      ...
      <div>
        ...
        <div className="border-2 border-dashed border-gray-300 dark:border-gray-700 rounded p-6 text-center cursor-pointer hover:border-gray-400 transition-colors" onClick={addFile}>
          {uploadedFiles.length > 0 ? (
            <div className="space-y-3">
              {uploadedFiles.map((f, i) => (
                <div key={i} className="flex items-center justify-center gap-2 text-gray-600 bg-gray-50 dark:bg-gray-700 rounded p-2">
      ...
      <div>
        ...
        <textarea rows={4} value={caption} onChange={e => setCaption(e.target.value)}
          className="w-full px-4 py-2.5 rounded border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm resize-none focus:ring-2 focus:ring-gray-500 outline-none"
      ...
      <div>
        ...
        <input type="text" value={hashtag} onChange={e => setHashtag(e.target.value)}
          className="w-full px-4 py-2.5 rounded border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-gray-500 outline-none" />
      </div>
      <div className="flex gap-3">
...
function SubmitLinkForm({ task, onClose }: { task: typeof kolTasks[0]; onClose: () => void }) {
  ...
  if (step === 'done') {
    return (
      <div className="space-y-4 text-center py-4">
        ...
        <div className="p-4 bg-gray-50 dark:bg-gray-700 rounded text-left space-y-2 text-sm">
          <div className="flex justify-between"><span className="text-gray-500">Chiến dịch</span><span className="font-medium text-gray-900 dark:text-white">{task.campaignName}</span></div>
          <div className="flex justify-between"><span className="text-gray-500">Link</span><span className="font-medium text-gray-600 truncate max-w-[180px]">{postUrl}</span></div>
          ...
          <div className="flex justify-between"><span className="text-gray-500">Ảnh chụp</span><span className="font-medium text-gray-600">✓ Đã tải lên</span></div>
        </div>
        <Button onClick={onClose} className="w-full">Đóng</Button>
      </div>
    );
      ...
      <div className="space-y-4">
        ...
        <div className="rounded border border-gray-200 dark:border-gray-700 overflow-hidden">
          ...
          <div className="p-4 bg-white dark:bg-gray-800">
            ...
            <p className="text-sm text-gray-700 dark:text-gray-200 mb-2 whitespace-pre-wrap">{captionPreview}</p>
            <div className="flex items-center gap-3 text-gray-400 text-xs mt-2 pt-2 border-t border-gray-100 dark:border-gray-700">
        ...
        <div className="p-3 bg-gray-50 dark:bg-gray-700 rounded space-y-1 text-sm">
          <div className="flex justify-between"><span className="text-gray-500">Link bài đăng</span><span className="text-gray-600 font-medium truncate max-w-[180px]">{postUrl}</span></div>
          ...
          <div className="flex justify-between"><span className="text-gray-500">Ảnh chụp</span><span className="text-gray-600 font-medium">✓ Đã tải lên</span></div>
        </div>
        <div className="flex gap-3">
  ...
  return (
    <div className="space-y-4">
      <div className="p-3 bg-gray-50 dark:bg-gray-700 rounded border border-gray-200 dark:border-gray-700 text-xs text-gray-600 dark:text-gray-400">
      ...
      <div>
        ...
        <div className="border-2 border-dashed border-gray-300 dark:border-gray-700 rounded p-6 text-center cursor-pointer hover:border-gray-400 transition-colors"
...
function SubmitMetricsForm({ task, onClose }: { task: typeof kolTasks[0]; onClose: () => void }) {
    ...
    return (
      <div className="space-y-4 text-center py-4">
        ...
        <div className="p-4 bg-gray-50 dark:bg-gray-700 rounded text-left space-y-2 text-sm">
          ...
          <div className="flex justify-between"><span className="text-gray-500">Engagement Rate</span><span className="font-bold text-gray-600">{er}%</span></div>
  ...
  if (step === 'preview') {
    ...
    return (
      <div className="space-y-4">
        <div className="p-3 bg-gray-50 dark:bg-gray-700 rounded border border-gray-200 dark:border-gray-700 text-sm text-gray-600 dark:text-gray-300">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-white dark:bg-gray-800 rounded border border-gray-200 dark:border-gray-700 p-4 text-center">
            ...
            <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5 mt-2">
              <div className="bg-gray-500 h-1.5 rounded-full transition-colors" style={{ width: `${viewsPct}%` }} />
            </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded border border-gray-200 dark:border-gray-700 p-4 text-center">
            ...
            <p className="text-2xl font-bold text-gray-600">{er}%</p>
            <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5 mt-2">
              <div className="bg-gray-500 h-1.5 rounded-full transition-colors" style={{ width: `${erPct}%` }} />
            </div>
          ...
          <div className="overflow-x-auto">
            ...
            <tbody>
              {[
                ...
                { label: 'Likes', value: l, target: '-', color: 'text-gray-600' },
                { label: 'Comments', value: co, target: '-', color: 'text-gray-600' },
                { label: 'Shares', value: s, target: '-', color: 'text-gray-600' },
                { label: 'Saves', value: sa, target: '-', color: 'text-gray-600' },
                { label: 'ER', value: `${er}%`, target: `${targetER}%`, color: 'text-gray-600', isString: true },
              ].map((row, i) => (
                <tr key={i} className="border-b border-gray-100 dark:border-gray-700">
        ...
        <div className="bg-gray-50 dark:bg-gray-700 rounded p-3 text-center">
          <div className="w-32 h-32 bg-gray-200 dark:bg-gray-700 rounded flex items-center justify-center mx-auto mb-1">
  ...
  return (
    <div className="space-y-4">
      <div className="p-3 bg-gray-50 dark:bg-gray-700 rounded border border-gray-200 dark:border-gray-700 text-xs text-gray-600 dark:text-gray-300 flex items-center justify-between">
      ...
      <div className="grid grid-cols-2 gap-3">
        {[
          ...
          { label: 'Likes *', value: likes, setter: setLikes, placeholder: 'VD: 3000', icon: <Heart className="w-4 h-4 text-gray-400" /> },
          { label: 'Comments *', value: comments, setter: setComments, placeholder: 'VD: 400', icon: <MessageCircle className="w-4 h-4 text-gray-400" /> },
          { label: 'Shares', value: shares, setter: setShares, placeholder: 'VD: 200', icon: <Share2 className="w-4 h-4 text-gray-400" /> },
        ].map(f => (
          <div key={f.label} className="relative">
            <label className="block text-xs text-gray-500 mb-1">{f.label}</label>
            <input type="number" value={f.value} onChange={e => f.setter(e.target.value)}
              className="w-full pl-8 pr-4 py-2.5 rounded border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-gray-500 outline-none"
      ...
      <div className="p-4 bg-gray-50 dark:bg-gray-700 rounded text-center">
        <p className="text-xs text-gray-600 dark:text-gray-400 mb-1">Engagement Rate tự động tính</p>
        <p className="text-3xl font-bold text-gray-600">{er}%</p>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-gray-50 dark:bg-gray-700 rounded border border-gray-200 dark:border-gray-700 p-5">
          <p className="text-sm text-gray-600 dark:text-gray-400 mb-1">Đang chờ thanh toán</p>
          <p className="text-2xl font-bold text-gray-600">{(currentKOL.pendingPayment / 1000000).toFixed(1)}M ₫</p>
        ...
        <div className=" rounded border border-gray-200 dark:border-gray-700 p-5">
          ...
          <p className="text-2xl font-bold text-gray-600">{(paid.reduce((s, p) => s + p.paidAmount, 0) / 1000000).toFixed(1)}M ₫</p>
            ...
            <tbody>
              {kolPayments.map(p => (
                <tr key={p.id} className="border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/30">
                  ...
                  <td className="px-6 py-4 text-sm font-bold text-gray-600">{p.paidAmount.toLocaleString()}₫</td>
                  <td className="px-6 py-4">
      ...
      <div className="bg-white dark:bg-gray-800 rounded border border-gray-200 dark:border-gray-700 p-5">
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          ...
          <div className="flex justify-between"><span className="text-gray-500">QR thanh toán</span>
            <Badge label={currentKOL.qrUploaded ? 'Đã tải' : 'Chưa tải'} colorClass={currentKOL.qrUploaded ? 'bg-gray-100 text-gray-600' : 'bg-gray-100 text-gray-600'} />
          </div>
        </div>
      </div>
      ...
      <Modal isOpen={showWithdraw} onClose={() => setShowWithdraw(false)} title="Rút tiền" width="max-w-md">
        <div className="space-y-4">
          <div className="p-4 bg-gray-50 dark:bg-gray-700 rounded border border-gray-200 dark:border-gray-700 text-sm text-gray-600 dark:text-gray-300">
          ...
          <div>
            ...
            <input type="number" defaultValue={currentKOL.pendingPayment} className="w-full px-4 py-2.5 rounded border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-gray-500 outline-none" />
          </div>
          <div className="flex justify-end gap-3">
        ...
        <div className="bg-white dark:bg-gray-800 rounded border border-gray-200 dark:border-gray-700 p-6">
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={item.label} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700 rounded">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white dark:bg-gray-800 rounded border border-gray-200 dark:border-gray-700 p-5">
            ...
            {isEditing ? (
              <textarea rows={3} value={profileData.bio} onChange={e => setProfileData({ ...profileData, bio: e.target.value })}
                className="w-full px-4 py-2.5 rounded border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm resize-none" />
            ) : (
              ...
              <div>
                ...
                {isEditing ? (
                  <select value={profileData.contentCategory} onChange={e => setProfileData({ ...profileData, contentCategory: e.target.value })}
                    className="w-full px-4 py-2.5 rounded border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm">
              ...
              <div>
                ...
                {isEditing ? (
                  <input type="email" value={profileData.email} onChange={e => setProfileData({ ...profileData, email: e.target.value })}
                    className="w-full px-4 py-2.5 rounded border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm" placeholder="email@example.com" />
                ) : (
          ...
          <div className="bg-white dark:bg-gray-800 rounded border border-gray-200 dark:border-gray-700 p-5">
            ...
            <div className="border-2 border-dashed border-gray-300 dark:border-gray-700 rounded p-6 text-center cursor-pointer hover:border-gray-400 transition-colors"
              ...
              {qrUploaded ? (
                <div>
                  <div className="w-20 h-20 bg-gray-100 dark:bg-gray-700 rounded flex items-center justify-center mx-auto mb-2">
                    <QrCode className="w-12 h-12 text-gray-400" />
                  </div>
                  <p className="text-sm text-gray-600 font-medium">QR đã được tải lên</p>
